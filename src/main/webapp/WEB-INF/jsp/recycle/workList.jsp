...
<%--<script
        src="${pageContext.request.contextPath }/js\jquery.parsley\dist\parsley.min.js"
        type="text/javascript"></script>
<script
        src="${pageContext.request.contextPath }/js\jquery.parsley\src\extra\dateiso.js"
        type="text/javascript"></script>--%>
<script
    ...
    function getLayTables() {
        ...
        layui.use('table', function (data) {
            ...
            table.render({
                elem: '#worker'
                , filter: 'worker'
                , url: '/worker/getRecycleWorker.action'
                ...
                , cols: [[
                    ...
                    , {title: '操作',width:270, align: 'center', toolbar: '#barDemo'} //这里的toolbar值是模板元素的选择器
                ]]
                , response: {
                    statusCode: '200'
                }
...
//layer.msg('ID：'+ data.id + ' 的查看操作');
                    ...
                    layer.confirm('确定要永久删除该行数据吗？', function (index) {
                        ...
                        layer.close(index);
                        //向服务端发送删除指令
                        var uri = '${pageContext.request.contextPath}/worker/deleteWorkerActually.action';
                        var id = data.id;
                        $.ajax({
                            url: uri,
                            type: 'post',
                ...
                else if (layEvent === 'rollback') { //删除
                    layer.confirm('确定要恢复该行数据吗？', function (index) {

                        obj.del(); //删除对应行（tr）的DOM结构，并更新缓存
                        layer.close(index);
                        //向服务端发送删除指令
                        var uri = '${pageContext.request.contextPath}/worker/rollbackWorker.action';
                        var id = data.id;
                        $.ajax({
                            url: uri,
                            type: 'post',
...
<script type="text/html" id="barDemo">
    ...
    <div class="layui-btn-group">
        <c:if test="${fn:contains(user.role, '1')}">
            <a class="layui-btn layui-btn-xs" lay-event="rollback">恢复</a>
        </c:if>
    </div>
