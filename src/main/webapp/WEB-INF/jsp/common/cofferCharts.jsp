...
<script type="text/javascript">
    $(function () {
        $.ajax({
            url: '/coffer/cofferCharts.html',
            type: 'post',
            dataType: 'json',
            success: function (result) {
                getCofferCharts(result);
            }
        });
    });
    $("#submitBtn").on("click", function (e) {
        ...
        $.ajax({
            url:"/coffer/getCofferChartsByPage.action",
            type:'post',
            dataType:'json',
            data:{
                "startStr":startStr,
    ...
    function getCofferCharts(result) {
        var moneyArr = [], dateArr = [], operatorArr = [], depositArr = [];


        for (var i = 0, data = result, len = data.length; i < len; i++) {
            moneyArr.push(data[i].nowMoney);
            dateArr.push(dateFormat(data[i].createDate));
            operatorArr.push(data[i].operator);
            depositArr.push(data[i].lastMoney);
        }
